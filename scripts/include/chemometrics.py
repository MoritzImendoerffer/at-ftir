...
import string, scipy, copy, scipy.linalg
...
def pls(xdata,ydata,mask,factors,stb=None,type=0):
    ...
    >>> import scipy
    >>> xdata=scipy.array([[ 0.6 ,  0.57,  0.59,  0.81,  0.45],[ 0.96,  0.76,  0.99,  0.08,  0.17],[ 0.99,  0.06,  0.99,  0.28,  0.98],[ 0.2 ,  0.02,  0.35,  0.12,  0.34],[ 0.36,  0.02,  0.02,  0.48,  0.07],[ 0.5 ,  0.5 ,  0.59,  0.26,  0.81],[ 0.71,  0.07,  0.37,  0.09,  0.53],[ 0.2 ,  0.88,  0.48,  0.53,  0.93],[ 0.63,  0.44,  0.47,  0.33,  0.02],[ 0.63,  0.45,  0.23,  0.5 ,  0.59]])
    >>> ydata=scipy.array([[ 0.48],[ 0.55],[ 0.74],[ 0.43],[ 0.52],[ 0.95],[ 0.7 ],[ 0.23],[ 0.08],[ 0.42]])
    >>> mask=scipy.array([[0],[2],[0],[1],[0],[1],[1],[0],[2],[0]])
    ...
    for xi in range(factors):
        ...
        b=scipy.dot(scipy.transpose(output['Q']),scipy.transpose(scipy.dot(output['W'],
              scipy.linalg.inv(scipy.dot(output['P'],output['W'])))))
    ...
    output['b']=scipy.dot(scipy.transpose(output['Q'][0:ind+1,:]),scipy.transpose(scipy.dot(output['W'][:,0:ind+1],
          scipy.linalg.inv(scipy.dot(output['P'][0:ind+1,:],output['W'][:,0:ind+1])))))
